...
版本:`2026-08-01.03`(與 `SKILL.md` 的 `version` 同步——每次改動任一檔,兩檔版號一起更新成當天日期 + 當日流水號 `YYYY-MM-DD.NN`)。
...
- **程式化 QA**:掃 `<a:t>` 開頭殘留上述號碼 / 圈號 → 阻擋;該用編號的清單(目錄、有序步驟)確認段落真的有 `buAutoNum`;跨頁清單確認續頁有 `startAt`**且數值 = 前頁最後編號 + 1**(不是只查屬性存在——要算出前頁編到第幾號、比對續頁 `startAt` 是否接對;算錯會出現「5. 之後接 1.」這種明顯錯亂)。
